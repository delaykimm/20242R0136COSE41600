...
# SOR과 ROR의 차이점 시각화를 위한 포인트 클라우드 생성
sor_points = set(ind)  # SOR에서 유지된 포인트들의 인덱스
ror_points = set(ind)  # ROR에서 유지된 포인트들의 인덱스

# SOR에만 있는 포인트들
sor_only = sor_points - ror_points
sor_only_pcd = voxel_downsample_pcd.select_by_index(list(sor_only))
sor_only_pcd.paint_uniform_color([1, 0, 0])  # 빨강색
print(len(ror_points))

# ROR에만 있는 포인트들
ror_only = ror_points - sor_points
ror_only_pcd = voxel_downsample_pcd.select_by_index(list(ror_only))
ror_only_pcd.paint_uniform_color([0, 1, 0])  # 초록색
print(len(ror_points))

# 공통된 포인트들
common_points = sor_points.intersection(ror_points)
common_pcd = voxel_downsample_pcd.select_by_index(list(common_points))
common_pcd.paint_uniform_color([0, 1, 1])  # 보라색
...
# 차이점 시각화
visualize_point_clouds([sor_only_pcd, ror_only_pcd, common_pcd],
                      window_name="Differences: SOR only (Red), ROR only (Green), Common (Blue)",
                      point_size=2.0)
