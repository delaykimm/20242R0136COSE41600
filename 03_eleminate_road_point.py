...
voxel_size = 0.03  # 필요에 따라 voxel 크기를 조정
...
# RANSAC을 사용하여 평면 추정  (개선된 알고리즘 : MLESAC - 이상치 탐지 good, Octree 기반 추출 - 대규모 데이터 처리 )
plane_model, inliers = ror_pcd.segment_plane(distance_threshold=0.15, # 추정 결과를 확인하여 parameter 조정
                                             ransac_n=3,
                                             num_iterations=2000)
